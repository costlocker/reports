...
// copy-pasted from https://github.com/kucharovic/czech-holidays
final class CzechHolidays
{
    ...
    private static function calculateEaster($year)
    {
        if (extension_loaded('calendar')) {
            ...
            $easterDays = (int) ($pfm + $tmp + 1); // Easter as the number of days after 21st March
        }
        $march21st = DateTimeImmutable::createFromFormat('Y-m-d', sprintf('%4d-03-21', $year));
        return $march21st->add(new DateInterval('P' . $easterDays . 'D'));
    }
}
